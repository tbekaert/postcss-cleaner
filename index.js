...
module.exports = postcss.plugin('postcss-css-cleaner', function(opts) {
  ...
  if (Array.isArray(opts.sources)) {
    ...
    // error because sources is not an array and raw is not defined
  }
  opts.ignore = [/[\d]*%/].concat(opts.ignore ? opts.ignore : []);

  return function(root, result) {
    if (!opts.raw) {
      result.warn(`Array of sources or raw source is necessary to run this plugin. See doc.`)
      return;
    }
    root.walk(rule => {
      if (rule.type == 'rule') {
        let selectors = rule.selector.replace(/[\n]/, '').split(',');
        ...
        testSelectors.map((sel, i) => {
          ...
          if (!isIgnored) {
            ...
            if (!isFound) {
              if(selectors[i]){
                log('removedRules', [
                  `Remove selector '${selectors[i]}' line ${
            ...
            log('ignoredRules', [
              `Ignore selector '${selectors[i]}' line ${rule.source.start.line}`
            ]);
          }
        });
